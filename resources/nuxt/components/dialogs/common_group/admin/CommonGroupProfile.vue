...
export default {
  ...
  methods: {
    // 共有グループユーザー
    async getBelongCommonGroups(id) {
      try {
        // グループユーザー取得
        const option = { id: id, join_model: 'common_group' }
    ...
    async edit() {
      try {
        const res = await this.$axios.get(
          '/api/common_group/admin/' + this.propsSelectedObj.id
        )
      ...
      // 共有グループユーザー取得
      this.getBelongCommonGroups(this.selected.id)
    },
    // データ編集(書き込み)
    async update() {
      try {
        const res = await this.$axios.put(
          '/api/common_group/admin/' + this.selected.id,
          this.selected
        )
        // 対象一覧の更新
