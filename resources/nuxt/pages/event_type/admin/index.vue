    ...
    <div class="management-tablewrap">
      ...
      >
        ...
        <template v-slot:item.color="props">
          <view-color :color-code="props.item.color" />
        </template>
      ...
      <!-- 削除確認ダイアログのレイアウト -->
      <data-destroy
        props-model-url="/api/event_type/admin/"
...
import EventTypeProfile from '~/components/dialogs/event_type/admin/EventTypeProfile'
...
export default {
  ...
  data() {
    return {
      ...
      // テーブルヘッダ
      headers: [
        { text: '予定タイプ', align: 'center', value: 'name' },
        { text: 'カラー', align: 'center', value: 'color' },
  ...
  methods: {
    ...
    async indexDatas(search = null) {
      ...
      try {
        const res = await this.$axios.get('/api/event_type/admin', {
          params: options
        })
